...
if(isset($_POST['clear']) && $_POST['clear'] == 'reset') {
    if (isset($_COOKIE['Tape'])) {
        setcookie('Tape', '');
    }
} elseif (isset($_COOKIE['Tape'])) {
    $aJob->un_serialize($_COOKIE['Tape']);
    $edit=TRUE;
}
 ...
 */
//Build array that maps paper type to paper color comma seperated and ties it to a unique paper index
...
<div id="inner_container">
    <p style="font-size:12px;"><a href="<?echo ($aJob->isAdmin) ? 'admin.php">Admin' :'index.php">Home' ?></a> >> Tape Bind
                                  <? echo $edit ? '>> Job: '.$aJob->jobId : ''?></p>
    <div><form method="post" action="tape.php"><button id="clear" type="submit" value="reset" name="clear">Clear</button></form>
        <form name="aForm" id="aForm" method="post" action="tape_verify.php" enctype="multipart/form-data">
